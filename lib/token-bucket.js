...
const _mutateState = Symbol('mutateState');
...
class TokenBucket {
    constructor(params){
        ...
        this[_timestampProvider] = params.timestampProvider || (() => +new Date());
    }
    ...
    take(amount, callback){
        ...
        storage.getAndLock(id, function onGetAndLock(err, state, setAndUnlock){
            ...
            if (newState.count < 0){
                return setAndUnlock(function onErrorSetAndUnlock(err){
                    return callback(null, false);
                });
            }

            return setAndUnlock(newState,
